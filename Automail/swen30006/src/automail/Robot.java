...
public class Robot {
    ...
    public enum ModeType {OVERDRIVE, NORMAL}
  ...
  public void step() throws ExcessiveDeliveryException {    	
	  ...
	  switch(current_state) {
		...
		case RETURNING:
            ...
            if(current_floor == Building.MAILROOM_LOCATION){
            	...
            	/** If the robot is not at the mailroom floor yet, then move towards it! */
                current_floor = mode.update(current_floor, Building.MAILROOM_LOCATION, false, this);
            	break;
            }
		case WAITING:
            /** If the StorageTube is ready and the Robot is waiting in the mailroom then start the delivery */
		...
		case DELIVERING:
			if(current_floor == destination_floor){ // If already here drop off either way
        		...
        		/** The robot is not at the destination yet, move towards it! */
                current_floor = mode.update(current_floor, destination_floor, false, this);
			}
            break;
	}
}
    ...
    public void changeMode(ModeType modeType) {
    	if(modeType == ModeType.NORMAL) {
    		mode = new Normal();
    	} else if(modeType == ModeType.OVERDRIVE)
